...
- **`catalog-lite.json` fields:** `file, title, year, work_type, themes, keywords, motifs, description, series, favorite, featured, orientation` (`orientation` = vertical/horizontal/square from `dims.json`, added session 35 — drives the archive orientation filter). Source of truth is `LITE_FIELDS` in `build_catalog.py`. Don't add fields without checking what `search.js` and the Netlify edge function actually use.
