...
- **Session 36 work is BUILT + preview-verified but NOT yet deployed/committed** (as of this writing). Deploy via JFSN.app (HTML/JSON/1,084 regenerated pages) + lftp the 3 recompressed hero AVIFs (see below). Then `bash end-session.sh`.
- **Latest committed commit at session-35 close was `d3d382e7`** (the start prompt was right; the old "da819efb" here was one commit stale — end-session ordering gap). GitHub + 4TB + Mac all verified holding `d3d382e7` at session-36 start.
- **B2 catch-up:** at session-36 start the B2 daily cap was STILL exceeded (verified 403 at ~4:50 PM EDT; cap resets 8 PM EDT). A wake-locked one-off job (`caffeinate -i … sleep 13800 && bash cloud-backup.sh`, started 4:27 PM) was alive and queued to fire ~8:17 PM EDT after the reset. **Next session: confirm that job actually ran and B2 now holds the latest commit** — if not, run `bash cloud-backup.sh` (cap will have reset). NOTE: the only recurring backup (`com.jfsn.backup` LaunchAgent, 11 PM) runs `backup.sh` = 4TB rsync ONLY, not B2 — B2 has no schedule, just that one-off. Consider making B2 recurring.
- **Live sw.js on jfsn.com = `jfsn-20260613190000`** (verified at session-36 start). After the pending deploy it becomes `jfsn-20260614010023` (auto-bumped by build_catalog when the catalog gained provenance fields).
...
- **NEW finding (not yet addressed): desktop CLS = 0.147** (Lighthouse, live old code) — mobile CLS is fine (0.001). Layout-shift culprit unidentified (need the expanded Lighthouse "Layout shift culprits" detail). Logged in IMPROVEMENTS 🟢.

## What was done session 36 (2026-06-13 — mobile hero LCP + provenance fields; BUILT + preview-verified, deploy pending)
