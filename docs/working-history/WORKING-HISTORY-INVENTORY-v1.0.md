...
9. **`site008`** — no confirmed hazards found in the survey pass only; per the `site002`/`site003`/`site004`/`site005` lesson, this should be read as "not yet closely inspected," not "confirmed clean," until it receives its own detailed record.
...
4. **`site004` / `site005` / `site006` / `site008` / `site009`** (professional-portfolio lineage) — lowest urgency; career-history value is real but not time-sensitive the way the family and fine-art threads are. **`site006` in particular now also carries a preservation-risk reason to move it up** (see Preservation Priority above) and should be inspected with the `site003` overlap specifically in mind. **`site004`'s and `site005`'s close-pass records are now both complete** (`docs/working-history/records/site004.md`, `site005.md`, 2026-07-05). `site005`'s pass did not find the "jfsn2007" gap-site material `site004` flagged — no reference to it was found in `site005`'s files — so that lead remains open for `site006`'s eventual pass or direct testimony. `site005` did surface its own new lead: whether its CGI Notify content is the same client engagement as `site002`'s nested `notify/` subsite, unresolved and worth checking whenever either record is revisited.
...
- **`site005`/`site002` relationship — new, unresolved:** whether the CGI Notify material on `site005` and the nested `notify/` subsite on `site002` document the same client engagement or two separate ones. Not yet compared directly (would require a dedicated diff between the two, not testimony) and not yet asked of Jeff either.
