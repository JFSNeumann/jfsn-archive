...
revision_history: Established 2026-07-12 (Jeff Neumann Archive). Amended 2026-07-19 (§8.3, forward-only applicability — see amendment note in-section).
---
...
**Amendment, 2026-07-19 (Jeff Neumann, Creator — per §11.2):** This document was orphaned from every documentation entry point (`CLAUDE.md`, `docs/README.md`, all other governance files) from its effective date, 2026-07-12, until 2026-07-19 — no prior session could have discovered it to comply. `config/catalog.json`'s 1,084 records, generated by `artworks/catalog.py` via the Anthropic API, carry no §8.3 transparency marker.

**Decision: §8.3 applies forward-only from 2026-07-19.** The existing 1,084 records are grandfathered as-is, not retroactively backfilled — the rule didn't findably exist when they were made. Any cataloging or metadata-generation work from this date forward is expected to comply with §8.3's marking requirement. Prior language above is unchanged; this amendment adds scope, it does not weaken §2's founding principles.
