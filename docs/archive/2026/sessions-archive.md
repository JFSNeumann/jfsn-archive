# JFSN Session Archive
**Frozen snapshot of the old CURRENT_STATE.md, preserved as a historical session-by-session log.**

This file used to be `CURRENT_STATE.md`. As of 2026-06-22, the live `CURRENT_STATE.md` was trimmed to actually describe current state (URLs, stats, known issues, critical open items) — about 30 lines instead of 504. The session-by-session narrative that previously sat in `CURRENT_STATE.md` is preserved below, untouched, as a read-only archive. Future sessions should NOT append to this file; new session work goes into commit messages and `IMPROVEMENTS.md` "Completed" highlights.

The full record of every session lives in `git log`.

---

# Current State (frozen 2026-06-20 14:45)
...
- **Fix:** added a new accessible token, `orange-ink` (`#B84700`, 5.07:1 AA), to `tailwind.config.js`. Swapped it in everywhere orange text sits on a **light** background. Left the original vivid `#FF6600` untouched everywhere it's a fill, border, hover/focus-only state, or text already on a **dark** background (Lost banner, homepage Stories card, lightbox/slideshow overlays, river "you are here" year chips) — those already passed at 6.7:1, and `#B84700` would have *failed* there (3.69:1). Checked each dark-bg case individually rather than blind-replacing (caught and reverted two early mistakes: the `.lr-btn` lightbox toolbar and a mini-river year chip, both on `rgba(11,11,11,...)` overlays that a simple `#0B0B0B` grep missed).
- **Scope:** all 38 hand-written pages, `_shared/top-nav.html`, the `tools/generators/gen-artwork-pages.py` template, and all 1,084 regenerated static artwork pages.
- **Focus-visible fix:** `archive.html`'s sort `<select>` had `outline:none` with **zero replacement** — a real keyboard-accessibility gap. Removed the override, added `:focus-visible` outline rules for it and the custom checkbox filters (also `appearance:none`, same problem).
- **Icon-button audit:** scanned every icon-only `<button>` and `<a>` sitewide for missing `aria-label` — zero gaps found, already solid.
- **CACHE_V bumped** to `jfsn-20260616195432`; `site.min.css` rebuilt to compile the new `text-orange-ink` utility.
- Preview-verified: computed color confirmed `rgb(184,71,0)` on light-bg elements, `rgb(255,102,0)` on dark-bg elements (both correct), zero console errors. `audit-nav.sh` 11/11 clean.
- **DEPLOYED + verified live on both hosts:** committed (`4f66d666`, 1,130 files), pushed, HostGator deploy confirmed live (cache-bust version + CSS rule + class match exactly), Netlify deployed via the full `--check` → draft → `--prod` workflow and verified live (same CACHE_V, fix present, credential PDF still correctly 404s).
- **CLAUDE.md updated** with the new `orange-ink` token and a contrast rule so future sessions don't reintroduce the bug.
- 🔴 **B2 cloud backup still capped** (`transaction_cap_exceeded`) — retried twice this session, both failed. GitHub + 4TB are current; B2 needs the daily cap to reset.
...
- **What it is:** all 1,084 works laid out in a 6-decade × 4-medium grid (small color swatches, using each work's dominant tone from `chromatic.json`), with 8 theme filter chips (Guernica, Targets, Framed, Torsos & Faces, Gallery, Mr. Snowmann, Crosses, Collaboration) above it. Clicking a chip dims everything else to 12% opacity and orange-outlines the matches — instantly shows where a recurring theme actually concentrates (which decades, which mediums), rather than just a flat filtered list. Verified Guernica → exactly 232 matches (matches the documented count), visibly concentrated in 1990s–2010s Collage, almost none in Photography/Painting — a real, useful pattern, not just a visual trick.
- **Real bug caught during verification, not shipped:** "Guernica" filter initially matched 0 works. Found the cause — Guernica is tracked in catalog-lite.json's `series` field, not the `themes` array (the other 7 chips are themes; Guernica is a named series). Fixed by combining `themes` + `series` into one matchable tag set per work before the chip-click handler ever ran. Re-verified: 232 matches, correct.
- **Built on existing infrastructure, not new assets:** fetches `catalog-lite.json` + `chromatic.json` (both already exist, no new data pipeline); hover/click/tooltip/thumbnail behavior is the same pattern as the Chromatic River (consistency, not a new interaction language); header uses the same orange-eyebrow/warm-brown-border/bracket-CTA/reading-progress-bar pattern as collage.html and the migrated decade pages.
- **Wired into the site properly, not just dropped in:** added to `stamp-nav.sh` TARGETS (nav/footer will stay in sync going forward), added to `artworks/build_catalog.py`'s sitemap `entries` list and reran it (sitemap now 1,130 URLs), added a "Curatorial Map" link to `_shared/footer.html`'s Resources column (both desktop and mobile blocks) — ran `stamp-nav.sh` afterward so all 38 pages picked up the new footer link in one pass. `audit-nav.sh` clean (sitemap coverage, link checks, all pass).
- **Verification note:** screenshots of the open page rendered correctly this time (no repeat of the slideshow-overlay screenshot artifact from earlier — that was specific to a full-viewport fixed overlay, not a general issue). Confirmed via DOM: all 1,084 swatches render (none dropped to an unexpected medium/decade bucket), theme-chip toggle on/off both work, mouse hover tooltip+thumbnail work, click navigates to the right artwork page. Tested at mobile width too — chips wrap, grid stays legible at the smaller swatch size. Zero console errors throughout.
- **No new Tailwind utilities** — inline `<style>` block only (same approach as every other Stitch page), no CSS rebuild needed.
- **CACHE_V bumped** to `jfsn-20260616211500` in sw.js.
- **🟡 NEEDS DEPLOY:** `bash end-session.sh` then JFSN.app — this is a new page, so confirm post-deploy that `curatorial-map.html` itself is live (not just referenced) before considering this done.
- **All three "wow factor" features Jeff asked for are now built**: artwork.html slideshow mode, chromatic.html touch scrub, and this Curatorial Map page.
...
- **#1 shipped: full-archive autoplay slideshow on `artwork.html`.** New `[ ▶ Play sequence → ]` trigger in the metadata aside (next to the existing `[ ← → Browse Works ]` hint) opens a full-screen dark overlay that autoplays through the entire 1,084-work `catalog.json` array (already fetched by the page, no new network cost) starting from the current work, advancing every 4.5s with the same saturate-developing crossfade (`.artwork-developing`) used for normal page loads. Controls: `[ ← PREV ]` / `[ ▶ PLAY ]`/`[ ❙❙ PAUSE ]` / `[ NEXT → ]` / `[ VIEW WORK ]` (jumps to that work's real detail page) / `[ ✕ CLOSE ]`, all styled with the same `.lr-btn` bracket-button class as the full-resolution lightbox (visual consistency, not a new button language). Keyboard: ← → step manually (pausing autoplay), Space toggles play/pause, Esc closes. `prefers-reduced-motion` respected — slideshow opens paused (no forced motion) instead of autoplaying immediately. Next/prev image preloaded one ahead/behind so advances feel instant.
- **Verification note:** preview screenshots of the open overlay rendered with a visible margin showing the underlying page — investigated and confirmed via direct DOM measurement (`getBoundingClientRect()` matched `window.innerWidth/innerHeight` exactly, twice, in two different overlay states) that this is a screenshot-tool rendering artifact, not a real layout bug. All functional behavior (open/close, play/pause, prev/next caption updates, progress bar advancing, Esc-to-close) was verified directly via DOM/JS state inspection instead, and all passed. Zero console errors.
- **No new Tailwind utilities** — inline styles + existing `.lr-btn`/`.bracket-link`/`.artwork-developing` classes only, no CSS rebuild needed.
- **CACHE_V bumped** to `jfsn-20260616194500` in sw.js.
- **🟡 NEEDS DEPLOY:** `bash end-session.sh` then JFSN.app.
- **Next up: #2, Chromatic River scrub-through on chromatic.html.**
...
- **Root cause:** the Google Fonts stylesheet (`<link href="https://fonts.googleapis.com/css2?...&display=optional" rel="stylesheet"/>`) is a plain synchronous stylesheet link — fully render-blocking on every single page, sitewide. This predates today's session entirely; it's just the first time it bubbled to the top of the insight list since the heavier hero/image issues were fixed.
- **Fix — converted to the standard preload+swap async pattern, sitewide:** `<link rel="preload" as="style" href="..." onload="this.onload=null;this.rel='stylesheet'"/>` + a `<noscript>` fallback for the (rare) no-JS case. This removes the font CSS from the render-blocking critical path — the browser still fetches it with high priority, but doesn't hold first paint hostage waiting for it. Pairs naturally with the existing `display=optional` (session 44): text paints immediately in the fallback font; Playfair/Inter swap in once the (now non-blocking) stylesheet finishes, with no layout shift either way.
  - Applied via a one-off Python script (written, run, deleted) across **1,122 files**: all ~38 hand-written pages, `tools/generators/gen-artwork-pages.py`'s template (so all future regenerated pages carry the fix), and all 1,084 already-generated static artwork pages. Verified 0 stragglers — every file containing the old pattern was converted, nothing missed.
  - Preview-verified desktop + a decade page + an artwork page: Playfair Display and Inter both render correctly (the swap-to-stylesheet trick works), zero console errors on any of the three.
- **No new Tailwind utilities** — no CSS rebuild needed.
- **CACHE_V bumped** to `jfsn-20260616170500` in sw.js.
- **🟡 NEEDS DEPLOY:** `bash end-session.sh` then JFSN.app — plain HTML edits across many files, no separate asset upload needed.
- **Ask Jeff for one more Lighthouse mobile run after this deploys.** If render-blocking is genuinely gone from the insights list and LCP stops swinging as wildly, that's the last piece. If LCP is *still* spiking after this, the remaining suspects are Lighthouse's own simulated-throttling run-to-run noise (real, not infinitely chasable) rather than another concrete site defect — at that point it may be worth accepting the achieved range (roughly 1.6–5s, down from 1.7–8.7s at the start of the day) rather than continuing to chase single-run outliers.

## ✅ SESSION 45 CONTINUED AGAIN (2026-06-16) — Selected Works mobile srcset, found via a 2nd round of Jeff's Lighthouse runs (READY TO DEPLOY)
- **Jeff re-ran Lighthouse mobile twice more after the hero recompression deployed.** Results: Performance 79/LCP 5.2s/CLS 0, then Performance 89/LCP 1.6s/CLS 0.087. Real improvement — worst case dropped from 8.7s to 5.2s, best case held at ~1.6–1.7s — but the swing didn't fully close, and **"Improve image delivery" (516–634 KiB savings) showed up in all four Lighthouse runs across both rounds**, before and after the hero fix, meaning it was never about the hero.
- **Root cause: the homepage's top-3 "Selected Works" cards load the 900px `medium/` crop with no `srcset`/`sizes`, on both desktop and mobile.** `#featured-grid` (`index.html` ~line 1127) is `grid grid-cols-1 md:grid-cols-12` — i.e. NOT hidden on mobile, just single-column there. At a ~345px mobile card width, a flat 900px source with no responsive alternative is exactly what that Lighthouse insight flags. This was a deliberate session-33 choice ("top 3 editorial keep medium" for quality) that never accounted for mobile not having its own grid.
- **Fix:** added `srcset="artworks/thumbs/${file} 400w, artworks/medium/${file} 900w" sizes="(max-width:767px) 92vw, (max-width:1023px) 480px, 620px"` to the `i < 3` branch of the dynamic card template (`index.html` ~line 1371). Desktop keeps the same `medium` quality it had (sizes resolves to 480–620px there, still picking the 900w candidate at typical desktop DPR). Cards 4+ were untouched — they already use `thumbs/` only, no change needed.
- **Honest caveat, same pattern as the wall.html micro-tier work in session 44:** verified in preview that the browser actually selects the smaller `thumbs` (400w) candidate at DPR 1 — but at DPR 2 (this preview's mobile emulation) and DPR 3 (Jeff's real iPhone 15 Pro), the *effective* pixel need at a 345px CSS card (690–1035px) is closer to the 900w candidate, so high-DPR phones will likely keep loading `medium` anyway. This genuinely helps standard-DPI displays and reduces the byte-savings gap Lighthouse measures, but may not fully clear that specific insight on Jeff's own test device. Zero regression either way — same `medium` fallback as before wherever `srcset` doesn't pick something smaller.
- **No new Tailwind utilities** — no CSS rebuild needed.
- **CACHE_V bumped** to `jfsn-20260616163000` in sw.js.
- Preview-verified at 375px/DPR2: `srcset` present and correct on all 3 cards, browser picks a candidate (not literally broken), zero console errors, screenshot confirms homepage renders unchanged.
- **🟡 NEEDS DEPLOY:** `bash end-session.sh` then JFSN.app — this is a plain HTML edit, no separate asset upload needed (unlike the hero AVIFs).
- **Ask Jeff for one more Lighthouse mobile run after this deploys.** The two Lighthouse insights still open after the hero fix were "Render-blocking requests" (560–890ms, Google Fonts — not addressed this session, would need self-hosting or a more invasive change) and "Improve image delivery" (addressed here, impact uncertain on Jeff's actual high-DPR phone per the caveat above). If LCP is still swinging meaningfully after this, render-blocking fonts is the next concrete suspect, not more guessing.

## ✅ SESSION 45 CONTINUED (2026-06-16) — Mobile LCP regression found + fixed via Jeff's Lighthouse run (READY TO DEPLOY)
- **Jeff ran Lighthouse on jfsn.com/index.html twice (mobile, simulated throttling) and got wildly different results: Performance 68/LCP 8.7s vs Performance 90/LCP 1.7s.** CLS was fine both times (0.03 / 0.087, both under the 0.1 "good" threshold — confirms the session-44 `display=optional` fix holds). The LCP swing was NOT noise.
- **Root cause: a real regression, not variance.** `git show 98fe8390` (2026-06-15, the Stitch homepage redesign session) shows the homepage's lead/preloaded hero image was swapped from `art0392` (46KB mobile crop — deliberately chosen in session 36 specifically *for being the lightest in the pool*, to fix this exact LCP-swing problem) to `art1010` "Cassette Torso" (170KB mobile crop — the 2nd-heaviest of all 10 images in the hero pool). The swap was an editorial choice for the redesign that didn't re-check the LCP-weight constraint it was overriding. Because the homepage hero rotation was retired in the same redesign (`MAX_SLIDES=1`), whichever image is line 1 of `featured-hero.txt` is now the **permanent, always-preloaded** LCP element — there's no averaging-out across a rotation anymore, so a heavy line-1 image is a permanent tax, not an occasional one.
- **Fix — asked Jeff which approach he wanted, since the two options have different visual consequences:** (a) revert to art0392 — zero risk, but changes the masthead image back; or (b) keep Cassette Torso as the masthead but recompress it. Jeff chose **(b) recompress**, to preserve the redesign's editorial choice.
  - Re-encoded `art1010-hero.avif` + `art1010-hero-m.avif` from the existing AVIFs (decoded via `magick` to PNG, since `avifenc` can't take AVIF as direct input) at **q35 / `--yuv 420` / speed 4** — same guarded approach as the session-36 hero recompression (only ship if it shrinks substantially with no visible quality loss).
  - **Mobile: 170,148 → 125,548 bytes (−26%)** — moved from 2nd-heaviest to mid-pack in the 10-image pool (now between art0250 and art1009). **Desktop: 964,735 → 509,595 bytes (−47%).**
  - Visually spot-checked both at normal viewing scale (full-bleed homepage masthead + a 600px comparison crop) — no visible compression artifacts; the collage's dense, busy texture hides q35's softening well.
  - Originals backed up to `artworks/_hero-orig-backup/art1010-hero.avif` / `-hero-m.avif` (matching the session-36 backup pattern for the other 3 recompressed heroes).
  - Preview-verified live: masthead still shows Cassette Torso correctly, zero console errors.
- **CACHE_V bumped** to `jfsn-20260616160500` — required this time (unlike most HTML/CSS edits) because the SW caches AVIFs cache-first by URL; without the bump, any visitor who already cached the old heavy `art1010-hero*.avif` under the previous CACHE_V would keep being served the stale 170KB/964KB versions indefinitely.
- **🟡 NEEDS DEPLOY:** the new `artworks/full/art1010-hero.avif` + `art1010-hero-m.avif` must be uploaded **flat to `/artworks/`** on HostGator (NOT `/artworks/full/`) — same hero-upload path as every other hero crop, per the `.htaccess` rewrite rule. `bash end-session.sh` then JFSN.app handles the rest of the site; the hero AVIFs need the separate flat lftp upload.
- **Ask Jeff to re-run Lighthouse mobile one more time after this deploys** — expect LCP to land somewhere between the 1.7s best case and the original 8.7s worst case, likely much closer to 1.7s now that the preloaded element is ~125KB instead of 170KB. If it's still spiking, the next suspect is render-blocking Google Fonts (flagged in both Lighthouse runs as "Render-blocking requests — Est savings 570ms–1990ms") or the "Improve image delivery" insight Lighthouse flagged in both runs (603KB–1285KB potential savings — worth a follow-up audit of what else is loading unoptimized on the homepage, separate from the hero).
...
- **🔴 DEPLOY INCIDENT (found + FIXED this session):** the first session-38 deploy **leaked `design-concepts/`** (Stitch mockups + 7 screenshots) to the public site — `deploy.sh`'s `--exclude-glob="design-concepts/"` does NOT match nested paths (files were under `design-concepts/stitch-june-2026/`). **Removed the folder from the live server (verified 404)** + switched `deploy.sh` to the regex `--exclude="design-concepts/"` form (commit `3029e4c6`). **Lesson: `artworks/verify_deploy.py` does NOT check for leaked private dirs** — when deploying after adding a new top-level folder, curl-check it's 404 on jfsn.com. (Pre-existing, NOT addressed: `old-site/` and `*.sh` scripts are also mirrored to HostGator — long-standing, low-risk since `.ftp.env` is excluded.)
...
- **Mobile hero LCP fix (warm-up).** Root cause confirmed by Jeff's Lighthouse: the first hero slide was random, so mobile LCP swung 1.3s–7.1s depending on which (heavy) slide loaded. Fixes: (1) `featured-hero.txt` now LEADS with **art0392** (Art Machine — lightest crop, 46 KB mobile / 133 KB desktop, and a real non-composite work); (2) `index.html init()` makes **slide 0 deterministic** (`[pool[0]].concat(shuffle(rest))`) so the LCP image is fixed; (3) media-aware `<link rel=preload as=image>` for slide-0's `-hero-m`/`-hero` variants, **stamped into `<head>` by `artworks/build_catalog.py`** from `featured-hero.txt` line 1 (HERO_PRELOAD markers) so it stays in sync. art0392's hero assets are already live (200) so the preload works on deploy. Preview-verified: art0392 leads both viewports, both preloads present, zero console errors. **Re-run Lighthouse AFTER deploy to confirm the mobile swing closes.**
...
- **Provenance fields (the main build).** Jeff's three decisions: (a) ALL years are estimates, display decade-bucket **"1990s (est.)"**; (b) composite set = **broadest sweep** (Gallery ∪ Studio ∪ placement-title); (c) skip `description_source`. Implemented in `artworks/build_catalog.py` (one uniform loop after the records sort): `year_precision='estimated'` + `year_display="<decade>s (est.)"` for all 1,084; `composite=True` for **250** works (`PLACEMENT_RE` + Gallery/Studio themes). Added all three to `LITE_FIELDS`. Display: `artwork.html` (dynamic) + `tools/generators/gen-artwork-pages.py` template both show `year_display` for the Year row + an **"Image — Photoshop composite — imagined placement"** meta row when `composite`. JSON-LD `dateCreated` kept as the numeric decade anchor (machine-parseable). **All 1,084 static pages regenerated.** api/v1 + catalog-lite now carry the fields (API honesty). Preview-verified: composite art0953 shows the note + "2000s (est.)"; non-composite art0392 shows neither. CACHE_V auto-bumped to `jfsn-20260614010023`. **Scope note:** grid/search/favorites captions still show the bare decade year (e.g. "1990") — only detail pages + API carry "(est.)". Extending to grids is an optional follow-up (would add visual noise; deferred).
...
- **Performance pass (5 items, deployed + verified live; from Jeff's Lighthouse runs: desktop 79 / mobile 74, mobile LCP 18.2s):**
  ...
  2. **Hero pool inlined** — `HERO_POOL:START/END` markers in index.html, stamped by artworks/build_catalog.py from artworks/featured-hero.txt (still the editing surface; rerun build_catalog after editing it). No featured-hero.txt fetch on the LCP critical path; parseTxt removed.
...
- **sw.js CACHE_V** — `artworks/build_catalog.py` auto-bumps only when catalog content changes. Manual bump required after HTML/CSS/JS edits without catalog rebuild.
...
## Backup status
**Last B2 backup:** 2026-06-16 23:32:29
