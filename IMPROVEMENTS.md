...
### Technical
- [ ] **Deploy sessions 36–39** — BUILT + preview-verified, not yet live. (1) JFSN.app deploy (index.html + all 31 stamped pages, sw.js CACHE_V bumped). (2) lftp the 3 recompressed hero AVIFs (`art0953/art1008/art1009-hero.avif`) to `/artworks/` flat — optional ~679 KB win, decoupled from LCP. (3) `bash end-session.sh`. (4) **Re-run Lighthouse mobile** to confirm LCP closed.
- [ ] **Next Stitch pages** — Stories oral-history rebuild (next pick per memory), then About, Artwork Detail, Wall. Stitch screens already built for all four.
