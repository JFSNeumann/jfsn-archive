...
- [ ] **DECISION NEEDED — should `_shared/anime.min.js` move into the shared `_shared/top-nav.html` template?** Found 2026-06-23: index.html carries a hand-authored `<script src="_shared/anime.min.js" defer>` tag *inside* its NAV:START/END block (Session 79's hero animation), which is not in the shared template. Because `stamp-nav.sh` overwrites that whole block verbatim, re-running it for any unrelated nav/footer fix silently deletes the tag — which happened during this session's Design System Modal removal and shipped live for several minutes before being caught and fixed (commits `277a4375` → `b99b35b1`). The tag has now been restored to index.html only. Two ways to make this durable: (a) add `<script src="_shared/anime.min.js" defer>` to `_shared/top-nav.html` itself, so every page loads it (small ~17KB deferred file, currently unused outside the homepage — low cost, but it's a sitewide behavior change someone should bless), or (b) leave it as a documented one-off divergence and rely on checking `git diff --stat` after every `stamp-nav.sh` run (the discipline already written up in this session's notes, which is exactly what failed this time). Not fixed unilaterally — pick (a) or (b).
...
### Technical
- [ ] **`style-guide.html` has its own orphaned nav, never wired into `stamp-nav.sh`** (found 2026-06-22 sweep) — it's a live, sitemapped page with a `NAV:START` marker, but its header is bespoke one-off HTML: a hidden (`display:none`) `<nav>` containing a single stale "Start Here" link, not the canonical `_shared/top-nav.html` structure. Not in `stamp-nav.sh`'s TARGETS, so it never got the current 4-link nav, the mobile drawer, or any nav update since whenever it was created. Needs a decision: wire it into stamp-nav.sh properly, or if it's a leftover prototype, consider removing it from the sitemap.
...
- [x] **17 Session-65 interaction scripts audited in full, 2026-06-22** (resolves the earlier same-day flag, which was based on a stale Don'ts list — corrected separately in `DESIGN-SYSTEM.md`; generic motion like parallax/scroll-reveal/scale-transform is NOT banned per `CLAUDE.md`'s actual current stance, so that was never the real question). Read every file and checked live attribute/class usage against each one. Findings:
  ...
  - Remaining open question for Jeff, not resolved here: whether to delete the ~8 confirmed-dead files outright (lower clutter, smaller payload) or leave them as dormant scaffolding for features that were planned but never wired up. Not deleted in this pass — per the project's own "don't delete in a hurry" stance, that's a product call, not a bug fix.
