...
| 5 | GitHub repo, current tip — `make_handoff.py:144` | Public repository, unauthenticated raw access | **CLOSED** — pushed session 30; re-verified 2026-06-12: raw tip serves the `.ftp.env`-reading version, 0 password occurrences |
...
4. Run `python3 make_handoff.py` → print the regenerated PDF for Allison's physical folder. Do not commit it (now gitignored).
5. Optional server cleanup (with approval): delete from the HostGator webroot: `make_handoff.py`, `gen-artwork-pages.py`, `curate-session.json` (all preserved elsewhere first — see SESSION-30 checkpoint §5; all already 403-blocked by .htaccess).
