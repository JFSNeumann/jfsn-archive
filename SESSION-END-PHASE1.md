...
- **What:** Read all core docs (`CLAUDE.md`, `DESIGN-SYSTEM.md`, `tailwind.config.js`, `package.json`), the build/data pipeline (`build_catalog.py`-adjacent docs, `gen-artwork-pages.py` references), the full nav/SW chain (`sw.js`, `_shared/top-nav.html`), `_shared/ui.js` in full, and ran systemic greps across all 38 root HTML pages and ~60 `_shared` modules (script-tag census, dead-file detection, inline-handler counts, FOUC-anchor check, etc.).
...
No implementation approach is prescribed here; that decision belongs to the next session, informed by reading `gen-artwork-pages.py` and this document's "Known Risks" section on generated-page workflows first.
