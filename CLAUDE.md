...
- **`catalog-lite.json` fields:** `file, title, year, work_type, themes, keywords, motifs, description, series, favorite`. Don't add fields without checking what `search.js` and the Netlify edge function actually use.
