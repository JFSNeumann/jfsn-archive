...
1. **Immediately:** the mirror at jfsn-archive.netlify.app keeps the archive publicly reachable. (To bring it current: push the repo to GitHub; Netlify redeploys automatically — pre-deployment checklist in `SESSION-30-FINAL-REMEDIATION-REPORT.md` §6.)
...
4. **Point the domain** (the hard part — see Scenario D if the friend is unreachable): at Gandi, change jfsn.com's nameservers/DNS to the new host. Until DNS moves, the archive lives at the new host's subdomain.
...
## Scenario D — The domain is lost or the friend is unreachable

1. Before expiry: anyone with the Gandi account can renew or transfer; ask the friend for a transfer authorization code — moving the domain into the family's own registrar account is routine and ends this scenario class forever.
2. If expiry passes: registrars hold domains in grace/redemption for ~30–75 days — call Gandi immediately; redemption is expensive but possible.
...
| Domain expiry | **2027-03-05** — renew yearly, registrar Gandi (friend's account) |
