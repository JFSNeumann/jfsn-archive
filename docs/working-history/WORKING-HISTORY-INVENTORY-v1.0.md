...
- **Type:** family-collaboration
- **Thread(s):** family-collaboration
- **Technologies (known):** Adobe Portfolio platform (`myportfolio.com`), captured via HTTrack Website Copier 3.49-2 — confirmed directly this session (`hts-log.txt`: 18 Nov 2021, from Windows path `C:\sebastians new test site\seb`). **No server-side code of any kind** — confirmed by direct search, architecturally unlike every Muse-based record in the collection. Individual gallery pages are titled by artwork subject ("Gorilla," "Dragon," "Moose," "Funny Grandma," "Untitled With Grandpa," plus newly confirmed: "Rhinoceros," "Parrot," "Elephant," "Nice Bird-Eating Spider," "Stick Man," "Face Collage," "Drawing," "Something About Poop," "Diplodocus," "Dragon team: Toothless"), plus continuity pages carried over from `site003` (`knex.html`, `aquabeads.html`)
- **Preservation concerns — confirmed directly this session, per `docs/working-history/records/site007.md`:** the New Relic finding is verified, not just carried over — a live inline agent script with a real license key/application ID that loads a remote script (`js-agent.newrelic.com`) and beacon (`bam.nr-data.net`) at runtime. **New this session:** `video-test.html` also embeds a live Adobe video-player iframe (`www-ccv.adobe.io`), a second live third-party call not previously documented. `hts-cache/winprofile.ini` was checked directly per HTTrack's own credential warning — no actual credentials found, only generic default config.
- **Current condition — refined:** true file count is 245 (not the "98 files" previously recorded), including a near-identical duplicate `jfsn.myportfolio.com/` mirror tree and seven Adobe CDN-mirror subfolders — a file-count refinement, not a substantive correction.
- **Historical significance:** high family/legacy value — documents the same collaboration maturing into a more developed, portfolio-style presentation. **New, specific cross-reference confirmed this session:** "Dragon team: Toothless" appears both as a full page here and as an image caption in `site003`'s `legos.html` — the most concrete title match found between any two family-collaboration records in this collection.
- **Relationships to other records:** confirmed continuation of `site003` (shared K'NEX/Aquabeads content, same subject; strengthened this session by the "Dragon team: Toothless" title match)
- **Relationships to artworks:** none found
...
- **Title (known, but incomplete):** "Bootstrap demo" (raw page `<title>` — **generic Bootstrap template default, never personalized in the source**, every page identical); visible on-page heading reads "Human-Centered Designer" (on `index.html`); other pages have project-specific headings ("Websites and Apps," "Mobile and Tablet Designs," "Contract Administration Tracking System," etc.)
- **Approximate year (known, from folder name):** 2020 — no conflicting metadata evidence found (unlike `site003`/`site005`, which had generator version strings contradicting the folder date)
- **Type:** professional-portfolio
- **Thread(s):** professional-design
- **Technologies — confirmed directly by close inspection, per `docs/working-history/records/site008.md`:** Bootstrap framework 5.3.2 (CDN-hosted), no server-side code, no custom CSS/JS beyond framework defaults, inline styles only. No forms, no handlers, no tracking. CDN dependencies: Bootstrap, Popper.js. One YouTube embed.
- **Preservation concerns — corrected from "scaffold-like" to "minimal but complete":** the "Bootstrap demo" `<title>` tag on every page does reflect incomplete personalization, but the site itself is fully functional and navigable, not broken. This is the **least technically hazardous of all nine sites** — zero preservation risk beyond standard CDN dependencies and the YouTube embed's link-rot potential. The minimal content structure (heading + intro paragraph + image gallery on most pages) reflects a template-based deployment rather than a heavily customized portfolio; whether this represents a deliberate stylistic choice or an incomplete build is **unknown**.
- **Current condition:** complete and fully navigable, 7 HTML pages (6 top-level + 1 in assets/), 100 images properly organized and referenced, all external links resolving, no broken dependencies as preserved. The term "scaffold-like" more accurately describes the content depth (minimal per-project narrative) than the technical completeness.
- **Historical significance:** documents a career-portfolio period (2020, post-site006/pre-site009) with less substantive per-project content than earlier portfolio records, but real, published portfolio material
- **Relationships to other records:** related to `site006` (2018) and `site009` (2023) as part of the ongoing portfolio lineage; not independently re-verified here, connection maintained from existing inventory
- **Relationships to artworks:** none found
...
9. **`site008`** — **closely inspected 2026-07-05, technical hazards confirmed absent** (see `docs/working-history/records/site008.md`). This is the **second record** where a close pass confirmed the survey's original finding rather than overturning it (the first being `site001`). Bootstrap-only, no server-side code, no forms, no tracking, CDN-hosted CSS/JS are the only external dependencies (not hazardous, but link-rot vectors). One YouTube embed present (standard preservation consideration, not a hazard). This record is the **least technically hazardous of all nine sites**.
...
3. **`site003` / `site007`** (Sebastian, both generations) — high family value; should be scheduled together since they document the same subject across time, and testimony about one will likely inform the other. **Both records' close-pass records are now complete** (`docs/working-history/records/site003.md`, `site007.md`) — `site003`'s relationship to `site006` is resolved as far as file comparison can take it (see Related Sites); `site007`'s New Relic hazard and continuation relationship to `site003` are both directly confirmed, strengthened by a specific "Dragon team: Toothless" title match. The remaining open question for this pair is when/how the drone-animation block was manually added to `site003`'s `sebastian.html`, which is a testimony question, not a file-inspection one.
4. **`site004` / `site005` / `site006` / `site008` / `site009`** (professional-portfolio lineage) — lowest urgency; career-history value is real but not time-sensitive the way the family and fine-art threads are. **`site004`'s, `site005`'s, `site006`'s, and now `site008`'s close-pass records are all complete** (`docs/working-history/records/site004.md`, `site005.md`, `site006.md`, `site008.md`). `site006`'s pass did not find the "jfsn2007" gap-site material `site004` flagged — no reference to it was found in `site006`'s files — so that lead remains fully open, not advanced by any record so far. `site006` also shares `site005`'s open "CGI Notify"/"CGI Bank" cross-reference question (both records reference the same two named projects); worth resolving whenever either is revisited. Only `site009` remains for this lineage, not yet given its own close-pass record.
...
- **`site003`/`site007` timing:** roughly when each Sebastian site was live, and roughly how much time passed between them. Unadvanced this session — `site007`'s own creation/live date remains unknown, only its 18 Nov 2021 capture date is established.
- **`site007` — new:** whether the Adobe video-player embed in `video-test.html` (`www-ccv.adobe.io/v1/player/ccv/FHimWgtGu78`) still resolves today, and what the video actually shows — not tested or asked.
...
- **`site008` — new, multiple open questions:** (1) whether this site was ever deployed live at `jfsn.com` or any domain, or existed only as a local work-in-progress; (2) why every HTML page retained the generic "Bootstrap demo" `<title>` tag — whether intentional (left as a placeholder), oversight, or evidence of incomplete work; (3) whether the categorization on this site (Web, Mobile, CATS, AI/UX, Misc) reflects Jeff's own professional focus areas at the time, or a generic template structure adapted to his portfolio; (4) what the embedded YouTube video (`jUwi6mCKakw`, visible in one of the image grid sections) actually shows — content and whether it is Jeff's own work or licensed content.
